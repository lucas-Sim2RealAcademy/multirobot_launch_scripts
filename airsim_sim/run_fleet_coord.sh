#!/usr/bin/env bash
# RUN A — field-faithful 4-drone coordinated exploration:
# per-drone real cuVSLAM+odom_correction+nvblox+FIS+planner (domains 1..N)
# + REAL coordination_stage (coordination_node, alignment, keyframe exchange;
#   embedded lora_bridge finds no radio -> as-flown degradation)
# + 4x zenoh-bridge-ros2dds on loopback with the as-flown generated configs.
# usage: run_fleet_coord.sh <N> [seconds]
set -e
N=${1:-4}; SECS=${2:-180}
