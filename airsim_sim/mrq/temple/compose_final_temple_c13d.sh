...
BAG=${BAG:?bag dir}
RATE=${RATE:-2}
OUT=/home/lucas/hercules-sim/mrq_fleet_temple3d.mp4
OUTM=/home/lucas/hercules-sim/mrq_fleet_temple3d_mobile.mp4
SCI_OUT=/home/lucas/hercules-sim/rviz_fleet_science3d.mp4
SCI_OUTM=/home/lucas/hercules-sim/rviz_fleet_science3d_mobile.mp4
mkdir -p "$C"
...
drawtext=fontfile=$FONT:text='HERCULES Fleet Exploration — 3D':fontsize=72:fontcolor=white:x=(w-tw)/2:y=h/2-110,
...
drawtext=fontfile=$FONT:text='4x autonomous 3D exploration (cuVSLAM + nvblox + FIS + 3D A*) - ground-truth trajectory replay':fontsize=26:fontcolor=0x76808f:x=(w-tw)/2:y=h/2+70" \
...
for line in "${SHOT_LINES[@]}"; do
  ...
  # 4-beat cut: this run cannot feed the fly-past or depth-stack grammars
  case "$name" in s2_throughline|s4_depthstack) continue;; esac
  n=$((end - start))
  seg="$C/seg_$name.mp4"
...
ffmpeg -y -loglevel warning -ss "$OFF" -t 40 -i "$RESHOT" -filter_complex "$MAPVF" \
  -map "[v]" -c:v libx264 -preset slow -crf 18 -pix_fmt yuv420p "$C/map_growth.mp4"
HOLD_SS=$(python3 -c "print(max(0.0, float('$DUR') - 5.5))")
ffmpeg -y -loglevel warning -ss "$HOLD_SS" -t 8 -i "$RESHOT" -filter_complex "
