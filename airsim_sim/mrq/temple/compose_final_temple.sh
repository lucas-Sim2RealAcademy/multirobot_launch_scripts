#!/usr/bin/env bash
# Temple cinematic composite: encode MRQ frames, cut the time-aligned RViz
# window, PiP composite, title card, then FULL + MOBILE deliverables.
# Time alignment identical to mrq_work/composite/compose_final.sh (rviz x11grab
# is CFR 30fps; start epoch = stop_epoch - nframes/30; offset = t0 - start).
#
# usage: compose_final_temple.sh <keys.json> <frames_dir> <rviz.mkv> <grab_stop.txt> <out_full.mp4> <out_mobile.mp4>
set -eu
KEYS=$1; FRAMES=$2; RVIZ=$3; STOPF=$4; OUT=$5; OUTM=$6
W=/home/lucas/UE5/hercules-sim-big/mrq_work/temple/composite
...
# 3) composite: main 1920x1080 + RViz PiP bottom-right + labels
...
# 5) mobile: 960x540, crf 26 (+2 per retry until <5MB), +faststart
crf=26
while :; do
  ffmpeg -y -loglevel warning -i "$OUT" -vf "scale=960:540" \
    -c:v libx264 -preset slow -crf $crf -pix_fmt yuv420p -movflags +faststart "$OUTM"
  sz=$(stat -c %s "$OUTM")
  echo "mobile crf=$crf size=$sz"
  [ "$sz" -lt 5000000 ] && break
  crf=$((crf+2))
  [ $crf -gt 36 ] && { echo "WARN: cannot reach <5MB"; break; }
done
echo "COMPOSITE_OK $OUT + $OUTM"
