#!/usr/bin/env bash
# FINAL composite v4 (user feedback pass):
#   1. title card -> CLEAN temple cinematic (NO coverage paint / ribbons / PiP):
#      establishing 0-15 s + hero-track 53.3-80 s  (~42 s of footage)
#   2. transition card -> merged-map RViz FULL-SCREEN for the remainder
#      (SPEED x with honest chip; captured live during the run)
# Also emits the standalone merged-map video (1x, full length) + mobiles.
#
# The map footage is the BAG RE-SHOT played at 2x (ros2 bag play -r 2), so the
# clip is already sped; we chip "2x speed" and do NOT setpts again.
set -eu
SPEED_LABEL="2x"
...
BAG0=1786745009.296848393          # ros2 bag info start (bag_mapview2)
...
# ---- 1) temple cinematic: two trims from the clean frames ----
ffmpeg -y -loglevel warning -framerate 30 -start_number 0 -i "$FRAMES/frame_%04d.png" \
  -frames:v 450 -c:v libx264 -preset slow -crf 18 -pix_fmt yuv420p "$C/cine_seg1.mp4"
ffmpeg -y -loglevel warning -framerate 30 -start_number 1600 -i "$FRAMES/frame_%04d.png" \
  -c:v libx264 -preset slow -crf 18 -pix_fmt yuv420p "$C/cine_seg2.mp4"

# labels on the cinematic (small, top-left only)
for seg in cine_seg1 cine_seg2; do
...
printf "file '%s'\nfile '%s'\nfile '%s'\nfile '%s'\nfile '%s'\n" \
  ...
  "$C/transition.mp4" "$C/map_seg.mp4" > "$C/concat.txt"
