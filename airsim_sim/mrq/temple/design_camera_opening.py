...
_sep_min = float(os.environ.get("S1_SEP_MIN", "288"))
ok1 &= gate(f"s1 pair separation >={_sep_min:.0f}px by t=6s", sep6 is not None and sep6 >= _sep_min,
...
if os.environ.get("S2_SKIP") == "1":
    ...
    for s in K["info"]["shots"]:
        if s["name"] == "s2_throughline":
            s["skip"] = True
    json.dump(K, open(DST, "w"))
    print(f"wrote {DST} (S2 SKIPPED per env, s1 rows installed)")
    sys.exit(0)
    ...
    for v in DRONES:
        ...
        for s in sxy:
            ...
            if inx and iny:
                ...
                break
    crossers = 2 * max(min(exits["L"], exits["R"]), min(exits["T"], exits["B"]))
