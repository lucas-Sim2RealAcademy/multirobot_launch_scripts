...
def scrape(label):
    ...
    try:
        ...
        # cuVSLAM stutter events/min per drone (field reference: <=2).  This is
        # the real VIO gate -- image_jitter_threshold_ms:=200.0 in
        # run_fleet_radio.sh:176 -- not a nominal 30 Hz frame period.
        st = []
        for ln in r.stdout.splitlines():
            f = ln.split()
            if len(f) >= 13 and f[0] in ('ghost', 'delta', 'buckshee',
                                         'thunderstrike'):
                try:
                    st.append(float(f[9]))
                except ValueError:
                    pass
        if st:
            out['stut'] = '/'.join('%.0f' % s for s in st)
            out['stut_max'] = max(st)
    except Exception:
        pass
    return out
...
def main():
    ...
    print('%-18s %-11s %6s %6s %6s %6s %6s  %5s %7s %6s  %s'
          % ('label', 'load1 pre/med', 'effort', 'unique', 'floor',
             'ovlp%', 'EXCESS', 'div', 'plan/ok', 'done%', 'stut/min'))
    print('-' * 112)
    for lb in labels:
        s = scrape(lb)
        ...
        print('%-18s %-11s %6d %6d %6.1f %6.1f %6.1f  %5s %7s %6s  %s'
              % (lb, s['load'],
